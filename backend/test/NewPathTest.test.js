...
function areAllEqual(arr) {
    if (arr.length === 0) return true; // Empty array, considered equal by default
...
async function CheckLocation(receivedData) {
    try {
        ...
        if(areAllEqual(data['Expected'] ,data['Queried'], data['HTML'] , data['Distance']) && data['Expected'] == 0){
            return { skipped : true};
        }
        //console.log(false);
        ...
        console.log(`${error}`);
        console.log(`${receivedData.MultiStopArray[0]} to ${receivedData.MultiStopArray[1]} : failed`);
        return { source: receivedData.MultiStopArray[0], destination: receivedData.MultiStopArray[1], passed: false , nodes_path : []  , HTML : '' , error_can_handle : false , skipped : false};
    }
}
...
class TestResult{
    ...
    log_progress(isSkipped){
        if(this.processed_count > 0 && this.processed_count % 50 == 0 && !isSkipped) {
            console.log(`${this.processed_count} test cases processed`);
        }
        if(this.skipped > 0 && this.skipped % 500 == 0) {
            console.log(`${this.skipped} test cases skipped`);
        }
    }
    ...
    async process_block_result(block_result , source , destination){
                ...
                for (let destination of locations) {
                    ...
                    if (source !== destination) {
                        ...
                        tasks.push(async () => {  
                            ...
                            if(non_block_result.skipped){
                                result.increment_skip();
                                result.log_progress(true);
                                return;
                            }else{
                                result.increment_test_cases();
                            }
                            ...
                            await result.process_block_result(block_result ,source , destination);
                            result.incre_process_count();
                            result.log_progress(false);
